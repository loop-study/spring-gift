...
### Step 5 — 로깅 횡단 처리 (AOP 도입) [추가]

**목표**: 각 서비스에 반복 선언된 `Logger` + `log.info` 패턴을 AOP로 횡단 처리하여, 서비스 코드에서 로그 코드를 제거한다.

**변경 내용**:
- `ServiceLoggingAspect` 생성 — `@within(Service)` 대상으로 메서드 호출 자동 로깅
- 전 서비스에서 `private static final Logger log` 선언 및 `log.info` 호출 제거
- `log.warn` (실패/경고 로그)은 서비스에 유지 — 비즈니스 판단이 필요한 로그는 AOP로 대체 불가

**검증**: 기존 테스트 전체 통과 + 로그 출력 확인
