...
class CategoryControllerTest extends IntegrationTest {
    ...
    void 카테고리를_생성한다() throws Exception {
        // given
        CategoryRequest request = createRequest("도서", "#8B4513", "https://example.com/books.jpg", "소설, 기술서적");
    ...
    void 카테고리를_수정한다() throws Exception {
        // given
        Long id = createCategoryAndGetId("수정전", "#000000", "https://example.com/before.jpg", "수정 전 설명");
        CategoryRequest updateRequest = createRequest("수정후", "#FFFFFF", "https://example.com/after.jpg", "수정 후 설명");

        // when
        ResultActions result = mockMvc.perform(put("/api/categories/" + id)
            .contentType(MediaType.APPLICATION_JSON)
    ...
    void 카테고리를_삭제한다() throws Exception {
        // given
        Long id = createCategoryAndGetId("삭제용", "#123456", "https://example.com/del.jpg", "삭제 테스트");

        // when
        ResultActions result = mockMvc.perform(delete("/api/categories/" + id));
    ...
    void 존재하지_않는_카테고리를_수정하면_404를_반환한다() throws Exception {
        // given
        CategoryRequest request = createRequest("없는카테고리", "#000000", "https://example.com/none.jpg", "없음");

        // when
        ResultActions result = mockMvc.perform(put("/api/categories/999999")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(request)));
    ...
    private Long createCategoryAndGetId(String name, String color, String imageUrl, String description) throws Exception {
        CategoryRequest request = createRequest(name, color, imageUrl, description);
        MvcResult response = mockMvc.perform(post("/api/categories")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
