...
class WishControllerTest extends IntegrationTest {
    ...
    void 위시리스트를_조회한다() throws Exception {
        ...
        // when
        ResultActions result = mockMvc.perform(get("/api/wishes")
    ...
    void 위시를_추가한다() throws Exception {
        ...
        String token = loginAndGetToken("user1@example.com", "password1");
        WishRequest request = createRequest(2L);

        // when
        ResultActions result = mockMvc.perform(post("/api/wishes")
            .header("Authorization", "Bearer " + token)
    ...
    void 중복_위시를_추가하면_기존_위시를_반환한다() throws Exception {
        ...
        String token = loginAndGetToken("user1@example.com", "password1");
        WishRequest request = createRequest(1L);
    ...
    void 위시를_삭제한다() throws Exception {
        ...
        // when
        ResultActions result = mockMvc.perform(delete("/api/wishes/1")
            .header("Authorization", "Bearer " + token));
    ...
    void 다른_사용자의_위시를_삭제하면_403을_반환한다() throws Exception {
        ...
        String token = loginAndGetToken("user2@example.com", "password2");
    ...
    void 존재하지_않는_상품에_위시를_추가하면_404를_반환한다() throws Exception {
        // given
        String token = loginAndGetToken("user1@example.com", "password1");
        WishRequest request = createRequest(999999L);
