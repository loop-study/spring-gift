...
- **관찰 가능성 누락**: AI는 테스트를 돌려서 "통과"를 확인하지만, 사람이 운영·디버깅할 때 필요한 로그는 스스로 필요하지 않기 때문에 자연스럽게 빠진다. 실제로 Phase 4에서 카카오 메시지 발송을 수동 테스트하려 했을 때, 로그가 전혀 없어서 정상 동작 여부를 확인할 수 없었다. 급히 Phase 5의 로깅 작업을 Phase 4로 당겨서 처리했다.
